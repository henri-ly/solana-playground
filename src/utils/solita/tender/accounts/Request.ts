...
export class Request implements RequestArgs {
  private constructor(
    ...
    readonly paymentMint: beet.bignum,
  ...
  static fromArgs(args: RequestArgs) {
    return new Request(args.price, args.paymentMint, args.state, args.bump)
  }
  ...
  pretty() {
    return {
      ...
      paymentMint: (() => {
        const x = <{ toNumber: () => number }>this.paymentMint
        if (typeof x.toNumber === 'function') {
          try {
            return x.toNumber()
          } catch (_) {
